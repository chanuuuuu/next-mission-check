...
function locateTeam(assignments: Record<string, number>, teamId: number): SeatGroup[] {
...
export default function ViewClient({ teams, assignments }: Props) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      const els = document.querySelectorAll<HTMLElement>(`[data-team-highlight="${matchedTeam.id}"]`)
      ...
      const targetScrollY = window.scrollY + blockCenterY - window.innerHeight / 2
      ...
      if (container) {
        const targetLeft = container.scrollLeft + blockCenterX - window.innerWidth / 2
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-12">
      <header className="border-b border-foreground px-4 py-4 sticky top-0 bg-background z-10">
        ...
        <h1 className="font-display text-base font-bold tracking-tight">내 좌석 찾기</h1>
      </header>
      ...
      <div className="px-4 py-5 space-y-5">
        ...
        {matchedTeam && located.length > 0 && (
          <div className="border border-foreground">
            ...
            >
              <span className="font-display text-lg font-bold">{matchedTeam.church_name}</span>
              {matchedTeam.team_name && (
                <span className="font-display text-sm text-foreground/70">{matchedTeam.team_name}</span>
              )}
