...
export default function AccommodationSearchPage() {
  ...
  const handleNext = () => {
    ...
    router.push(
      `/accommodation/${encodeChurchParam(selectedChurch.name, selectedChurch.id)}`,
    );
  };
  ...
  return (
    <div className="h-screen bg-muted flex justify-center overflow-hidden">
      <div className="w-full max-w-[480px] bg-background border-x border-foreground h-full flex flex-col animate-[var(--animate-slide-up)]">
        <header className="px-6 pt-4 pb-4 border-b border-foreground flex-shrink-0">
          <Link
            href="/"
          ...
          <h1 className="text-xl font-bold tracking-tight mt-4 leading-none">
            숙소 배정 조회
          </h1>
          <p className="text-sm text-muted-foreground mt-3">
            교회를 검색하여 배정된 숙소를 확인하세요.
          </p>
        </header>
            ...
            results.map((church) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  {church.address && (
                    <p
                      className={`text-xs mt-0.5 truncate ${selected === church.id ? "text-brand/70" : "text-muted-foreground"}`}
                    >
                      {church.address}
                    </p>
                  )}
